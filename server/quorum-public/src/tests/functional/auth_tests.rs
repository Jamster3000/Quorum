...
use crate::tests::common::{cleanup_user, create_test_user, get_test_username, make_auth_request};
use serde_json::json;
...
pub async fn test_delete_user_account_email() -> Result<TestResult, String> {
    let (username, password, user_id, _) =
        create_test_user("TestPassword123!", true, false).await?;
...
pub async fn test_delete_user_account_username() -> Result<TestResult, String> {
    let (username, password, user_id, _) =
        create_test_user("TestPassword123!", false, false).await?;
...
pub async fn test_refresh_token() -> Result<TestResult, String> {
    ...
    let timer = startup::create_timer();
    let refresh_body = make_auth_request(
        "/auth/refresh",
        &json!({
