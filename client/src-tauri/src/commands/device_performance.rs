...
pub async fn get_performance_tier() -> Result<String, String> {
    ...
    let swap_usage_percentage = if total_swap_mb > 0 { (used_swap_mb as f64 / total_swap_mb as f64) * 100.0 } else { 0.0 };
    ...
    let temp_penalty = if max_cpu_temp > 80.0 { 0.5 } // High temperature
                   else if max_cpu_temp > 60.0 { 0.2 } // Moderate temperature
                   else { 0.0 }; // No penalty
    let cpu_score = if cpu_cores >= 8 { 3.0 - temp_penalty }
                 else if cpu_cores >= 4 { 2.0 - temp_penalty }
                 else { 1.0 - temp_penalty };
    let cpu_speed_score = if cpu_speed_mhz >= 3000 { 3.0 }
                          else if cpu_speed_mhz >= 2000 { 2.0 }
                          else { 1.0 };
    let available_memory_mb = total_memory_mb - used_memory_mb;
    let swap_penalty = if swap_usage_percentage > 50.0 { 0.5 }
                   else if swap_usage_percentage > 20.0 { 0.2 }
                   else { 0.0 };
    let memory_score = if available_memory_mb >= 12_000 { 3.0 - swap_penalty }
                   else if available_memory_mb >= 6_000 { 2.0 - swap_penalty }
                   else { 1.0 - swap_penalty };
    let load_score = if load_avg < 1.0 { 3.0 }
                     else if load_avg < 2.0 { 2.0 }
                     else { 1.0 };

    // Weighted score
    let performance_score = (cpu_score * 0.3) + (cpu_speed_score * 0.2) + (memory_score * 0.3) + (load_score * 0.2);

    // Determine tier
    let tier = if performance_score >= 2.5 { "high" }
               else if performance_score >= 1.5 { "medium" }
               else { "low" };
